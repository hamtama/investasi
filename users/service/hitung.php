...
if(isset($_POST['ninja'])){
    ...
    $querybobot = $mysqli->query("SELECT * FROM tb_gejala JOIN tb_bobot ON tb_bobot.id_bobot = tb_gejala.id_bobot");
    $p = 1;
    ...
    $query_kasus = $mysqli->query("SELECT * FROM tb_rule");
    $o = 1;
    while ($qq = mysqli_fetch_array($query_kasus)){
        ...
        $cf['R'.$o]['id_kasus'] = $qq[9];
        $resdiagnosa['Rule'.$o]['id_kasus'] = $qq[9];
        ...
        while($qg = mysqli_fetch_assoc($gQue)){
            for($j=1; $j <= $cf['Rule'.$i]; $j++){
                if($j == 1){
                    ...
                    // echo "nilaiCF['Combine'".$xyz."] = ".$nilaiCF[$qg[$fetchCol[$j]]]['nilaiHE']." + ".$nilaiCF[$qg[$fetchCol[($j+1)]]]['nilaiHE']." x (1 - ".$nilaiCF[$qg[$fetchCol[$j]]]['nilaiHE'].")";
                    ...
                    $resdiagnosa['Rule'.$i]['persentase']= $nilaiCF['Combine'.$xyz]*100;
                    // echo "nilaiCF['Combine'".$xyz."] = ".$nilaiCF['Combine'.($xyz-1)]." + ".$nilaiCF[$qg[$fetchCol[($j+1)]]]['nilaiHE']." x (1 - ".$nilaiCF['Combine'.($xyz-1)].")";
                    // echo "<br>resdiagnosa['Rule'".$i."]['persentase']= ".$nilaiCF['Combine'.$xyz]." x 100";
                    $xyz++;
                    // echo $j;
                }
