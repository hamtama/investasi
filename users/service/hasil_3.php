...
for ($i=0; $i < sizeof($resdiagnosa); $i++){
    if($resdiagnosa[$i]['persentase'] != 0){
        if ($i==0 ){
            ...
            $query = $mysqli->query("SELECT * FROM tb_rule LEFT JOIN tb_kasus ON tb_rule.id_kasus = tb_kasus.id_kasus WHERE kode_rule = '$kode' AND tb_rule.id_kategori = '4'");
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                for ($j=1 ;$j<=$maxgejala; $j++){
                                                    $sql_g = $mysqli->query("SELECT $fetchCol[$j] FROM tb_rule WHERE id_rule = '$id_rule' AND id_kategori = '4'");
                                                    while($row_g = mysqli_fetch_array($sql_g)){
                                                        if ($row_g[0] != NULL){
                                                            echo '<tr><td>'.$row_g[0].'</td>';
                                                        }
                                                        $sql_pakar = $mysqli->query("SELECT `$fetchCol_2[$j]`,MAX(presentase) FROM tb_pengetahuan WHERE id_rule = '$id_rule' AND id_kategori = '4'");
            ...
            <?php 
                ...
                $query = $mysqli->query("SELECT * FROM tb_rule INNER JOIN tb_kasus ON tb_kasus.id_kasus = tb_rule.id_kasus WHERE kode_rule = '$kode' AND tb_rule.id_kategori = '4'");
                foreach ($query as $baris) {
                    $id_kasus = $baris['id_kasus'];
                $id_rule = $baris['id_rule'];
            ?>
