...
if(isset($_POST['ninja'])){
    ...
    $sql  = mysqli_query($mysqli, "SELECT * FROM tb_gejala WHERE id_kategori = '2'");
    ...
    $querybobot = $mysqli->query("SELECT * FROM tb_gejala JOIN tb_bobot ON tb_bobot.id_bobot = tb_gejala.id_bobot WHERE id_kategori = '2' ORDER BY id_gejala ASC ");
    $p = 1;
    ...
    $query_kasus = $mysqli->query("SELECT * FROM tb_rule WHERE id_kategori='2'");
    $o = 1;
    ...
    $xyz = 1;
    for ($i=1; $i <= $jumlah_kasus; $i++){
        $myQuery[$i] = $tesQuery."kode_rule = '".$cf['R'.$i]['kode_rule']."' AND id_kategori ='2'";
