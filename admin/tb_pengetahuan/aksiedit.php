...
echo "max gejala = ".$maxgejala;
$n_kriteria = "";
for ($i = 0; $i < $maxgejala; $i++){
    if($post[$i] != ''){
        $h = $i+1;
        $n_kriteria .= "gejala_$h = '".$post[$i]."'";
        if($i != $maxgejala-1){
            $n_kriteria .= ", ";
        }
    } 
}
echo "ID PENGETAHUAN = ".$id_peng." ";
echo "UPDATE tb_pengetahuan SET $n_kriteria, hasil = '$akar', id_kategori = '$kategori' WHERE id_pengetahuan = '$id_peng'";
$sql_insert = mysqli_query($mysqli, "UPDATE tb_pengetahuan SET $n_kriteria, hasil = '$akar', id_kategori = '$kategori' WHERE id_pengetahuan = '$id_peng'");
if($sql_insert){
    echo "input berhasil";
    ?>
