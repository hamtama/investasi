...
if (isset($_POST['rowid'] )) {
    ...
    $sql = "SELECT id_pengetahuan,a.id_kategori,$header, kategori, hasil FROM tb_pengetahuan a left join tb_kategori z ON z.id_kategori = a.id_kategori $join WHERE id_pengetahuan ='$id'";
	$result = $mysqli->query($sql);
	foreach ($result as $baris) {
?>
<form role="form" action="aksiedit.php" method="post" name="add_name" id="add_name">
    <input type="hidden" name="id_pengetahuan" value="<?=$baris['id_pengetahuan']?>">
    <?php
            $j = 1;
        ...
        <div class="col-md-9 col-sm-9">
            <select class="form-control has-feedback-left" name="kriteria<?= $id_kriteria ?>"
                ...
                <?php
                            ...
                            echo '<option value="'.$baris['id_gejala_'.$j].'">' . $baris['gejala_'.$j] . '</option>';
